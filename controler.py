...
def create_grp(participant, x, y, nb_part):
    list_grp = []
    for i in y:
        groupe = []
        import pdb;
...
def check_nb_part():
    ...
    while int(nb) > 8 or int(nb) == 0:
        ...
        try:
            nb = int(
                input("\033[32mVeuillez entrer le nombre de personne dans un groupe entre 1 et 8 personnes: \033[0m"))
...
def start():
    ...
    while choix != "q":
        ...
        if choix == "s":
            participant = ["Farid", "Marie", "Phichet", "Arthur", "Antoine", "Hatice", "Giovanni", "Mickael", "Rachid",
                           "Julien", "Vivien", "Kevin", "Josephine", "Valentin", "Camille", "Tanguy"]
            ...
            while A != "o" and A != "n":
                ...
                if A == "n":
                    create_grp(participant, x, y, nb_part)

                if A == "o":
                    comp(lvl, comp, participant)
